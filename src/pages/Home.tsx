...
export const Home = ({ onLogin }: { onLogin: () => void }) => {
  ...
  return (
    <div className="min-h-screen bg-surface-50 dark:bg-surface-950 text-surface-900 dark:text-surface-50 overflow-x-hidden font-sans">
      {/* Navigation */}
      <nav className="fixed top-0 w-full z-50 bg-white/80 dark:bg-surface-950/80 backdrop-blur-2xl border-b border-black/5 dark:border-white/5 px-4 md:px-10 py-3 md:py-5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="p-1.5 md:p-2.5 bg-gradient-premium rounded-xl md:rounded-2xl text-white shadow-xl shadow-emerald-500/20">
            <Building2 size={20} className="md:w-6 md:h-6" strokeWidth={2.5} />
          </div>
          <span className="font-black text-lg md:text-2xl tracking-tighter text-gradient leading-none">HomeConnect</span>
        </div>
        <div className="flex items-center gap-2 md:gap-10">
          <a href="#features" className="hidden lg:block text-[11px] font-black uppercase tracking-[0.2em] text-surface-500 hover:text-primary transition-colors">Features</a>
          <button 
            onClick={onLogin}
            className="px-4 md:px-8 py-2 md:py-3 bg-gradient-premium text-white rounded-xl md:rounded-2xl text-[10px] md:text-xs font-black uppercase tracking-widest shadow-2xl shadow-emerald-500/20 hover:scale-105 transition-all active:scale-95 whitespace-nowrap"
          >
            Open App
          </button>
        </div>
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-6 px-6 sm:px-0">
            ...
            <button className="w-full sm:w-auto px-8 md:px-12 py-5 md:py-6 bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl md:rounded-[2rem] font-black text-lg md:text-xl hover:bg-surface-50 transition-all flex items-center justify-center gap-3 md:gap-4 backdrop-blur-md active:scale-95">
              Explore Demo <Smartphone size={22} />
            </button>
          </div>
        </motion.div>
